...
def hp_extra_condition(pm):
    # If ignore_es is True (default), HP triggers regardless of ES
    if config["HP"].get("ignore_es", True):
        return True
    try:
        es_address = get_final_address(pm, base_address, offsets["CurES"])
        return pm.read_int(es_address) == 0
    except:
        return True
